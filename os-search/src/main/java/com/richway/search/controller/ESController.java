package com.richway.search.controller;
...
import com.richway.search.utill.ElasticsearchUtil;
import com.richway.search.utill.EsPage;
...
public class ESController {
     ...
     * @param
     */
    @RabbitListener(queues = RabbitMqQueue.PRODUCT_ROUTING_KEY)
    public void receiveProductMessageSpecific(EnterpriseAddress wrapper) {
        System.out.println(wrapper.getEntId());
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("id", DateUtils.formatDate(new Date()));
        jsonObject.put("age", 35);
        jsonObject.put("name", wrapper.getEntAddress());
        jsonObject.put("date", new Date());
        String id = ElasticsearchUtil.addData(jsonObject, "test_index1", "external1",wrapper.getId().toString());
...
//    }
    ...
    public String getAllData() {
        List list = ElasticsearchUtil.searchAll(indexName,esType);
